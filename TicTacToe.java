...
public class TicTacToe {
	...
	public static void main(String[] args) {
		...
		if(isValid(row,col)&&board[row-1][col-1]==' '){
			...
			player=++player%2;
			validMoves++;
		}else{System.out.println("Please provide valid inputs");}
	...
	public static boolean checkboard(char[][] board, int row, int col) {
		...
		boolean win=true;
		for(int i=0;i<3;i++){
			if(board[row][i]!=symbol){win=false;break;}
		}if(win){return true;}
		for(int i=0;i<3;i++){
			if(board[i][col]!=symbol){win=false;break;}
		}if(win){return true;}
		if(row==col){win=true;
		for(int i=0;i<3;i++){if(board[][]!=symbol){win false;}} if (win) {
			return true;}
			}}if(row+col==2){win=true;
	for(int i=0;i<3;i++){win=false;}
	// Buralar yanlış oldu karıştı
        return false;
    }
	...
	public static void printBoard(char[][] board) {
		...
		System.out.println("--------------");
		for(int row=0;row<3;row++){System.out.println(row+1+" ");
		for(int col=0;col<3;col++){System.out.println("|");
			System.out.println(" "+board[row][col]+" ");
		}if(col==2){System.out.println(" |");}}
